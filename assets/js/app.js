...
function questions(){
	...
	if (currentSubject == subjects.length ){
		...
		document.getElementById("questiongText").innerHTML ='Aangevinkte stellingen tellen extra mee bij het berekenen van het resultaat.'
...
function takePoliticalparty(){ 
	for(b = 0; b < answers.length; b++){
		for(a= 0;a<subjects[b].parties.length;a++){
			...
			// checks if your answer is equal to a party their position on the matter//
...
function answerDisplay() {
	...
	document.getElementById('checkboxList').style.display ='none'
	document.getElementById("blueText").innerHTML = 'Uw standpunten komen het meest overeen met: '
